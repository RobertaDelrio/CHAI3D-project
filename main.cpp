...
void reset(size_t taskId)
{
	...
	tasks[currentTask]->setInitialized(false);
	currentTask = taskId;
	...
	taskLabel = new cLabel(font);
	camera->m_frontLayer->addChild(taskLabel);

	// Initialize the current assignment
	tasks[currentTask]->initialize(world, camera);

	// Update the assignment label
	taskLabel->setText(tasks[currentTask]->getName());

	// Precalculate width to make it centered
	taskLabelWidth = taskLabel->m_font->getTextWidth(taskLabel->getText(), 0.01);
	tasks[currentTask]->setInitialized(true);
...
void updateGraphics(void)
{
	...
	if (tasks[currentTask]->isInitialized()){
		tasks[currentTask]->updateGraphics();
	...
	*/
	...
	taskLabel->setLocalPos(0.5 * (displayW - taskLabelWidth), displayH - 20, 0);
...
void updateHaptics(void)
{
	...
	while (simulationRunning)
	{
		...
		// Update assignment
		if (tasks[currentTask]->isInitialized())
			tasks[currentTask]->updateHaptics(hapticDevice.get(), timeStep, totalTime);
