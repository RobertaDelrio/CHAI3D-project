...

{
	...
	m_tool->setHapticDevice(a_hapticDevice);

	// if the haptic device has a gripper, enable it as a user switch
	//a_hapticDevice->setEnableGripperUserSwitch(true);
	...
	m_tool->m_hapticPoint->m_sphereProxy->m_material->setGray();
...
void cSetUp::updateHaptics(shared_ptr<cGenericHapticDevice> a_hapticDevice)
{
	...
	*/
	...
	m_tool->setLocalPos(position);
	m_tool->setLocalRot(rotation);

	//Change pov of the camera
	//m_cursor->setLocalPos(position.z(), position.y(), -position.x());
	m_tool->setLocalPos(position.z(), position.y(), -position.x());
	m_tool->setLocalPos(0.71*position.z()+0.71*position.x(), position.y(), 0.71*position.z() -0.71*position.x());
	/////////////////////////////////////////////////////////////////////
	// COMPUTE FORCES
	/////////////////////////////////////////////////////////////////////
...
void cSetUp::updateProtocol()
{
	...
	cVector3d position = m_tool->getGlobalPos();
	...
	case 3:{
		if (fabs(position.z() - endPosition.z()) < 0.005 && fabs(position.y() - endPosition.y()) < 0.005)
		{
			...
			if (m_tool->getLocalPos().x() < min) {

				min = m_tool->getLocalPos().x();
			}
	...
	case 4: {
		if (lev == 0)
		{
			...
			endp->m_material->setColor(pointColorEnd);
			error = min + (L1)+0.005;
			errop = error * 100 / 0.085; //error * 100) / double(L1);
			score = 100 + errop;
			if (error > 0 || score < 0) {
				score = 0;
			}
...
void cSetUp::updateLogging(void)
{
	...
	while (loggingRunning)
	{
		...
		trialFile << std::fixed << time << "\t"
			<< m_tool->getLocalPos().x() << "\t" << m_tool->getLocalPos().y() << "\t" << m_tool->getLocalPos().z() << "\t"
